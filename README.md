...
When you sign a spec, zense creates a `git worktree` for that session (branch `zense/impl/v<n>-<stamp>`, nested under `<repo>/.pi/zense/worktree/` inside your workspace — kept out of `git status` via a local entry in `.git/info/exclude`) and transparently redirects every `write`/`edit`/`read`/`bash` tool call into it. You can open **two pi sessions in the same repo** without them overwriting each other's files mid-work — each works in its own worktree. The grader/reviewer sub-agents run inside the worktree too, so they evaluate the real changes. When `zense_eval` **passes**, the worktree branch is auto-committed and merged back into `main` (`git merge --no-ff`) and the worktree is cleaned up. If another session already merged conflicting changes, the merge **escalates** (`need-decision`) and the worktree is left for you to resolve manually instead of silently clobbering. If a session ends before eval passes, the worktree is left in place (merge by hand: `git merge zense/impl/...`) — zense never auto-merges unverified work. `/zense status` shows the active worktree; the widget shows `🌳 <worktree>`.
