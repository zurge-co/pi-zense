...
(ไม่ค้าง?) กด **ctrl+_** หรือพิมพ์ `/zense agents` แล้วเลือก run → live tail อัปเดตเองทุก 1 วิ
...
**🌳 พอ eval PASS** harness squash ทุก commit ใน worktree branch เป็น commit เดียว (subject = spec title, body = intent + รายชื่อ interim commits) แล้ว **apply เข้า `main` แบบ staged-but-uncommitted** (`git merge --squash`) — **harness ไม่ commit ให้**: HEAD ของ main ไม่ขยับ ของทั้งหมดค้างอยู่ใน index ให้คุณ review จริงใน main ก่อน (`git status` / `git diff --cached`) แล้ว **คุณเป็นคน commit ชั้นสุดท้ายเอง** — สะดวกสุดด้วย message สำเร็จรูป `git commit -F .zense/pending-apply.msg` (แก้ไฟล์ตรงนี้ได้ตามใจ) หรือจะสั่ง agent commit ให้ก็ได้ พอรับงานแล้วให้บอก harness ด้วย **`/zense accept`** (หรือสั่ง agent เรียก `zense_accept` — ถ้าสั่งให้ agent commit แทนด้วย tool จะ commit จาก message ที่เตรียมไว้ให้เอง โดยให้ hook ทำงานปกติ): harness จะปิดบัญชี pendingApply, soft-verify ว่า commit เกิดขึ้นจริง (ถ้า index ว่างแต่ HEAD ไม่ขยับ = change น่าจะหายเงียบๆ จะเตือนแต่ไม่ refuse) และรายงาน **ชื่อไฟล์ที่คุณแก้เพิ่มหลัง grader ผ่าน** เป็นบทเรียนลง `/zense memory` — ถ้า review แล้วไม่พอใจ → **discard**: พิมพ์ `/zense discard` (หรือสั่ง agent เรียก `zense_discard`) เพื่อ reverse patch ที่เก็บไว้ (`.zense/pending-apply.patch`) คืน main เหมือนก่อน apply เป๊ะ โดยไม่แตะไฟล์อื่นของคุณ — และถ้าคุณไปแก้ไฟล์ที่ apply มาแล้ว discard จะ fail ชัดๆ แทนที่จะลบของคุณเงียบๆ สถานะนี้เรียกว่า **pendingApply**: `/zense status` แสดงพร้อมคำสั่ง commit/discard, widget ขึ้น `⏳staged v<n>` และ session ใหม่จะเตือนเองจนกว่าคุณจะ commit (index ยังไม่ durable — `git stash`/`reset --hard` ลบทิ้งได้) ถ้า main มี change ค้างอยู่แล้ว หรืออีก session merge ชน → harness **escalate** (`need-decision`) แล้วเก็บ worktree ไว้ให้ resolve เอง (ไม่ clobber/ไม่ปนของเงียบ) ถ้า eval **FAIL** เก็บ worktree ไว้แก้ต่อ ถ้าคุณปิด session ก่อน eval PASS เก็บ worktree ไว้ให้ merge ด้วยมือ (`git merge zense/impl/…`) — harness ไม่ auto-merge งานที่ยังไม่ verified
...
| `ctrl+_` หรือ `/zense agents` | ระหว่าง sub-agent รัน / ย้อนหลัง | ดู live tail ของ grader/reviewer กันสับสนว่าค้างไหม |
| `/zense accept` | หลัง review + commit แล้วรับงาน | ปิดบัญชี pendingApply + รายงานไฟล์ที่แก้เพิ่มหลัง grader ผ่านเป็น lesson (`accept commit` = ให้ harness commit แทนก่อน) |
| `/zense discard` | review แล้วไม่พอใจ | reverse patch คืนสภาพ main ก่อน apply เป๊ะ |
