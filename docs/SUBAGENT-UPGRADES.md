...
- **v3 (2026-09-02, grounding)** — รูรั่วจริง 2 จุดที่ทำให้ packet หลอน: (1) รอบ FAIL ค้าง escalation `need-fix` ไม่เคยล้างตอน PASS → reviewer เห็นปน evidence เขียน TL;DR ขัด verdict จริง → ล้าง need-fix ตอน PASS; (2) evidence ย่อเหลือ id:status → reviewer แต่ง identifier/hash เอง → reviewer prompt ได้ criteria/probe detail แบบ verbatim + grounding contract (hash/path/identifier ต้อง copy จาก evidence เท่านั้น) + `findUngroundedTokens` (pure, pathExists injectable) ตรวจ packet → retry×1 → trajectory flag `reviewer hallucination` · ถอด env override `ZENSE_SUBAGENT_TIMEOUT_MS` → ช่องของ agent คือ `.zense/config.json` key `subagentTimeoutMs` (อ่านสดต่อ launch, fallback worktree→main repo)
